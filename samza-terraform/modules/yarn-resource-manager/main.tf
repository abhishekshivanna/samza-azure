...
resource "azurerm_public_ip" "resource_manager_public_ip" {
...
resource "azurerm_network_interface" "resource_manager_nic" {
  name                      = "${var.prefix}-${var.rm-prefix}-nic"
  location                  = var.location
  resource_group_name       = data.azurerm_resource_group.resource_group.name
  network_security_group_id = data.azurerm_network_security_group.resource_manager_nsg.id

  ip_configuration {
    name                          = "configuration"
    subnet_id                     = data.azurerm_subnet.resource_manager_subnet.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.resource_manager_public_ip.id # TODO: Figure out a way not to use public IPs
  }
}

resource "azurerm_virtual_machine" "resource_manager_instance" {
